...
public class DublinCoreIndexer extends Indexer {
    ...
    public IndexingResult index(Path dcFile, Map<String, Path> dataFolders, final ISolrWriteStrategy writeStrategy, int pageCountStart) {
        ...
        try {
            ...
            addGroupDocs(indexObj, useWriteStrategy);
    ...
    PhysicalElement generatePageDocument(Element eleImage, String iddoc, String pi, final Integer inOrder, Map<String, Path> dataFolders) {
        ...
        if (!suppressFulltext && new FulltextAugmentor(dataRepository).addFullTextToPageDoc(ret.getDoc(), dataFolders, pi, order, null)) {
